...
function getPanelChildren(h, context) {
  ...
  if (context.mergedOptions.vuescroll.enableVirtual && context.hasCalculatedVirtualDomSizeAndPos) {
    viewStyle.height = context.vuescroll.state.virtualHeight + 'px';
    viewStyle.width = context.vuescroll.state.virtualWidth + 'px';
  }

  var children = virtualDomFilter.call(context, context.$slots.default);
...
function virtualDomFilter(slots) {
  ...
  var currentDom = this.vuescroll.state.currentViewDom;
  if (!this.mergedOptions.vuescroll.enableVirtual || !currentDom.length || !this.hasCalculatedVirtualDomSizeAndPos) {
    return slots;
  }

  // filtered dom which we will whow currently
  var filterdDom = currentDom.map(function (index) {
...
var core$1 = {
  ...
  computed: {
    ...
    contentElm: function contentElm() {
      return this.mode == 'slide' ? this.scrollPanelElm : this.mode == 'native' ? this.scrollContentElm : [];
    }
  },
  methods: {
    destroy: function destroy() {
